...
float FVector::SqrMagnitude(FVector v) { return v.X * v.X + v.Y * v.Y + v.Z * v.Z; }
float FVector::Magnitude(FVector v) { return sqrt(SqrMagnitude(v)); }
float FVector::Distance(FVector a, FVector b) { return FVector::Magnitude(a - b); }

struct FVector& FVector::operator+=(const float rhs) { X += rhs; Y += rhs; Z += rhs; return *this; }
struct FVector& FVector::operator-=(const float rhs) { X -= rhs; Y -= rhs; Z -= rhs; return *this; }
struct FVector& FVector::operator*=(const float rhs) { X *= rhs; Y *= rhs; Z *= rhs; return *this; }
struct FVector& FVector::operator/=(const float rhs) { X /= rhs; Y /= rhs; Z /= rhs; return *this; }
struct FVector& FVector::operator+=(const FVector rhs) { X += rhs.X; Y += rhs.Y; Z += rhs.Z; return *this; }
struct FVector& FVector::operator-=(const FVector rhs) { X -= rhs.X; Y -= rhs.Y; Z -= rhs.Z; return *this; }
