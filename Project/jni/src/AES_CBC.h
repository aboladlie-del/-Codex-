...
// Minimal AES-128 CBC Implementation
// Based on tiny-AES-c but simplified for this use case
...
// Support function for PKCS7/PKCS5 unpadding
static inline int aes_pkcs7_unpad(uint8_t* data, uint32_t len) {
    if (len == 0) return 0;
    uint8_t pad = data[len - 1];
    if (pad > 16 || pad == 0) return len;
    for (int i = 0; i < pad; i++) {
        if (data[len - 1 - i] != pad) return len;
    }
    return len - pad;
}
